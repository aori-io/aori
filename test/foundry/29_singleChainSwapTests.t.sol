...
contract SingleChainSwapTests is TestUtils {
    ...
    function testDepositThenFill() public {
        // Create order
        Order memory order = createSingleChainOrder(recipient, address(inputToken), INPUT_AMOUNT, address(outputToken), OUTPUT_AMOUNT);
        ...
        assertEq(
            ...
            initialOutputTokenRecipient + OUTPUT_AMOUNT,
            "Output tokens should be transferred to recipient exactly once"
        );
    ...
    function testDepositThenFillByDifferentSolver() public {
        ...
        assertEq(inputToken.balanceOf(userA), initialInputTokenUserA - INPUT_AMOUNT, "UserA input token balance should decrease");
        assertEq(
            outputToken.balanceOf(secondarySolver),
            secondarySolverOutputTokenBeforeFill - OUTPUT_AMOUNT,
            "Secondary solver output token balance should decrease"
        );
        assertEq(
            outputToken.balanceOf(recipient),
            initialOutputTokenRecipient + OUTPUT_AMOUNT,
    ...
    function testDepositThenFillWithExtraOutput() public {
        ...
        assertEq(
            outputToken.balanceOf(recipient), recipientBalanceBefore + OUTPUT_AMOUNT, "Recipient should receive exactly the output amount"
        );

        // Verify solver kept the surplus
        assertEq(
            outputToken.balanceOf(solver), solverBalanceBefore - OUTPUT_AMOUNT, "Solver should only transfer the exact amount in the order"
        );
    }
    ...
    function testDepositWithHookSuccess() public {
        // Create hook data
        bytes memory hookData = createHookData(address(outputToken), OUTPUT_AMOUNT);
    ...
    function testNoDoubleChargingInHookPath() public {
        ...
        assertEq(inputToken.balanceOf(userA), initialBalance - INPUT_AMOUNT, "User should only be charged input amount once");
    }
    ...
    function testNoDoubleTransferInFillPath() public {
        ...
        assertEq(outputToken.balanceOf(solver), initialSolverBalance - OUTPUT_AMOUNT, "Solver should only be charged output amount once");

        assertEq(
            outputToken.balanceOf(recipient), initialRecipientBalance + OUTPUT_AMOUNT, "Recipient should receive output amount exactly once"
        );
    }
    ...
    function testAllPathsConsistency() public {
        ...
        Order memory order1 = createSingleChainOrder(recipient, address(inputToken), INPUT_AMOUNT, address(outputToken), OUTPUT_AMOUNT);
        Order memory order2 = createSingleChainOrder(recipient, address(inputToken), INPUT_AMOUNT + 1, address(outputToken), OUTPUT_AMOUNT);
