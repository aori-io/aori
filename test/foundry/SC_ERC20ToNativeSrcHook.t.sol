...
contract SC_ERC20ToNativeSrcHook_Test is TestUtils {
    ...
    function testSingleChainERC20ToNativeWithSrcHookSuccess() public {
        ...
        console.log(
            "Flow: User deposits 10,000 tokens -> SrcHook converts to 1.1 ETH -> User gets 1 ETH, solver gets 0.1 ETH -> Atomic settlement"
        );
        console.log("");
    ...
    function testSingleChainSrcHookMechanics() public {
        ...
        assertEq(hookFinalTokens, hookInitialTokens + INPUT_AMOUNT, "Hook should receive input tokens");
        assertEq(hookFinalNative, hookInitialNative - HOOK_OUTPUT, "Hook should send native tokens");
    }
    ...
    function testSingleChainEventEmission() public {
        ...
        // Expect Deposit event (srcHook converted inputToken to NATIVE_TOKEN)
        vm.expectEmit(true, true, false, true);
        emit IAori.Deposit(expectedOrderId, order, NATIVE_TOKEN, HOOK_OUTPUT);

        // Expect Fill event (no dstHook in this test)
        vm.expectEmit(true, true, false, true);
        emit IAori.Fill(expectedOrderId, address(0), 0, 0);

        // Expect Settle event (for single-chain atomic settlement)
        vm.expectEmit(true, true, false, false);
        emit IAori.Settle(expectedOrderId, solverSC, 0, 0, address(0), 0);
