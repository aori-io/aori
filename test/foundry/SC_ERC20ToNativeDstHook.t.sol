...
contract SC_ERC20ToNativeDstHook_Test is TestUtils {
    ...
    function testCase12_ERC20DepositToDstHookNativeWithSurplus() public {
        ...
        console.log(
            "Flow: User deposits 10,000 tokens -> Solver fills with dstHook (1.2 ETH -> 1 ETH) -> User gets 1 ETH, solver gets 0.2 ETH surplus"
        );
        console.log("");
    ...
    function _testCase12Scenario(
        ...
        assertTrue(
            localAori.orderStatus(orderId) == OrderStatus.Settled, string(abi.encodePacked(scenarioName, ": Order should be settled"))
        );
